...
import clear_seen from "./routes/clear_seen.js";
...
// Clear the list of movies that have presented to the user.
app.use("/api/clear_seen", clear_seen);
...
app.post("/api/get_movie", (req, res) => {
  ...
  axios
    .get(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_watch_monetization_types=flatrate`
    )
    .then((response) => {
